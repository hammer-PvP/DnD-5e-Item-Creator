...
- Fixed **Recover Uses** and **Cost per Activation** remaining disabled after Maximum Uses was entered. Limited-use controls now unlock immediately from the same input event instead of waiting for an unrelated Activity Type/Unit rerender.
- Reorganized Activity configuration into clear **Identity & Activation**, **Targeting & Timing**, and **Uses & Recovery** groups. Numeric Range is only shown for measured-distance units, and numeric Duration is only shown for scalar time units; Instant/Special/Permanent durations no longer show a meaningless blank value field.
- Replaced the mid-form `Include`/`Disable` pair for composed Activities with a single **Enabled on Item** control in the Activity action header. Composed Activities are included by design unless explicitly disabled or removed.
- Removed Item Creator's custom root-level `inert`/darkening behavior while native child browsers are open. D&D5e already owns modal interaction blocking, and a stale custom inert state could leave the creator tinted and apparently frozen after a child-browser edge case. Render now also clears stale inert/busy state when no child browser is active.
- Activity card titles/summaries now update live while editing instead of waiting for an unrelated rerender.
- Added common Utility roll presets (`d20`, d20 + each Ability modifier, d20 + Proficiency) and reusable **Formula Helper** insertion for Utility, Damage, Heal, and Save formulas. This avoids requiring GMs to remember paths such as `@abilities.str.mod`.
- Clarified that a native **Damage Activity rolls damage directly and does not make an attack roll**. Special Weapon attacks continue to use the dedicated Attack Activities editor in Base Item.
- Weapon materialization now orders managed Attack Activities before composed Additional Activities and assigns composed Activities later native sort values, so an extra Damage/Utility power no longer appears above the weapon's primary Attack in the D&D5e Activity chooser.
- The Item Creator document schema remains **18**; these are UI/runtime construction corrections to the existing Activities v2 schema.
