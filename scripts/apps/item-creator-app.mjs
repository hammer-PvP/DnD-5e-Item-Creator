...
const UTILITY_ROLL_PRESETS = Object.freeze({
  none: "",
  d20: "1d20",
  str: "1d20 + @abilities.str.mod",
  dex: "1d20 + @abilities.dex.mod",
  con: "1d20 + @abilities.con.mod",
...
const FORMULA_TOKEN_OPTIONS = Object.freeze([
...
function utilityRollPreset(formula) {
  const normalized = String(formula ?? "").trim();
  const match = Object.entries(UTILITY_ROLL_PRESETS).find(([, value]) => value === normalized);
...
function composedActivityView(entry) {
  ...
  return {
    ...
    rangeNeedsValue: !["self", "touch", "spec", "any"].includes(source.range?.units ?? "self"),
    ...
    durationNeedsValue: (source.duration?.units ?? "inst") in (CONFIG.DND5E.scalarTimePeriods ?? {}),
...
export class ItemCreatorApp extends HandlebarsApplicationMixin(ApplicationV2) {
      ...
      .map(entry => {
        ...
        return {
          ...
          utilityPresetOptions: fixedOptions([
            ["none", "No Formula Roll"],
            ["d20", "d20"],
            ["str", "d20 + Strength Modifier"],
            ["dex", "d20 + Dexterity Modifier"],
            ["con", "d20 + Constitution Modifier"],
          ...
          formulaTokenOptions: fixedOptions([["", "Insert Modifier…"], ...FORMULA_TOKEN_OPTIONS], ""),
          saveAbilityOptions: configOptions(CONFIG.DND5E.abilities, row.saveAbility),
          saveCalculationOptions: [
            { value: "", label: "Fixed / Custom Formula", selected: row.saveCalculation === "" },
            { value: "spellcasting", label: "Spell Save DC", selected: row.saveCalculation === "spellcasting" },
            ...Object.entries(CONFIG.DND5E.abilities ?? {}).map(([value, config]) => ({ value, label: `8 + Proficiency + ${localizedLabel(config, value)} Modifier`, selected: row.saveCalculation === value }))
          ],
  ...
  _onRender() {
    ...
    if (root instanceof HTMLElement) {
      root.inert = false;
      const childOpen = Boolean(this.templateBrowserOpen || this.spellBrowserOpen || this.triggerBrowserOpen || this.iconBrowserApp);
      if (!childOpen) {
        root.classList.remove("ic-child-browser-open");
    ...
    root.querySelectorAll('[data-action="insert-composed-formula-token"]').forEach(select => select.addEventListener("change", event => this.#insertComposedFormulaToken(event)));
  ...
  #setBrowserBlock(active) {
    ...
    // Native D&D5e browser windows are already modal. Do not set the entire
    // Item Creator root to `inert`: if a child browser closes through an
    // unexpected route, a stale inert state makes the creator look permanently
    // frozen. Track busy state for accessibility/diagnostics only and let the
    // platform own modal interaction blocking.
    element.inert = false;
  ...
  #syncComposedActivityLiveControls(id, entry, data) {
    ...
    const title = details.querySelector('[data-composed-activity-title]');
  ...
  #insertComposedFormulaToken(event) {
    ...
    const id = select.dataset.composedActivityId;
    const field = select.dataset.formulaField;
    if (!token || !id || !field) { select.value = ""; return; }
    const details = this.element?.querySelector(`[data-composed-activity-details][data-composed-activity-id="${id}"]`);
    const input = details?.querySelector(`[data-composed-activity-input="${field}"]`);
  ...
  #updateComposedActivity(event) {
    ...
    switch (part) {
      ...
      case "enabled": entry.included = true; entry.disabled = !Boolean(value); break;
