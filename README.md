...
Post-roll Item Creator effects wait for higher-priority D&D5e and Character Builder decisions. With Character Builder `v0.9.8v` or later, Item Creator listens to `dnd5e-character-builder.rollResolutionPending` / `dnd5e-character-builder.rollResolutionFinalized` and consumes `game.modules.get("dnd5e-character-builder").api.rollResolutionQueue`. The finalized `currentTotal` is authoritative for chaining bonuses, but `target` and `succeeded` are never used to decide whether the Item Creator player prompt appears. If Bardic Inspiration changes 17 to 24, an eligible Item Creator bonus is still offered from 24; if it changes 10 to 18, the next eligible modifier starts from 18. This prevents the presence or absence of a later popup from revealing whether a previous result succeeded. When Character Builder marks a roll as pending but does not finalize it, Item Creator cancels its own offer for transaction safety rather than guessing the current total. If no higher-priority resolution is registered, Item Creator proceeds from the native roll total. Older integrations can still report a structured resolution through `game.itemCreator.reportRollResolution(data)` or the legacy shared queue symbol.
...
`https://github.com/hammer-PvP/DnD-5e-Item-Creator/releases/download/v0.5.0j/item-creator.zip`
